...
class SitePurchaseItemLine(models.Model):
    ...
    def onchange_product_id(self):
        for rec in self:
            item = self.env['site.purchase.item.line'].search([('item_id', '=', rec.item_id.id)], limit=1,
                                                              order='id desc')
            if rec.item_id:
...
class SitePurchase(models.Model):
        ...
        for rec in self:
            if rec.max_expected_date < rec.min_expected_date:
                raise ValidationError(
                    _("Maximum Expected Date must be equal or greater than Order Date and Minimum Expected Date"))
    ...
    def cancel_purchase(self):
        if self.state == 'order' or self.state == 'received':
            purchase_order = self.env['purchase.order'].search(
                [('state', 'in', ['confirmed', 'approved']), ('site_purchase_id', '=', self.id)])
            if purchase_order:
                raise except_orm(_('Warning'), _("PO Generated. Cancel PO First"))
            request_quiot = self.env['purchase.order'].search(
                [('state', 'in', ['draft', 'bid', 'sent']), ('site_purchase_id', '=', self.id)])
            for req in request_quiot:
                req.state = 'cancel'
            purchase_comp = self.env['purchase.comparison'].search(
                [('state', 'not in', ['cancel']), ('mpr_id', '=', self.id)])
    ...
    def button_create_comparison(self):
        ...
        if datetime.strptime(self.max_expected_date, "%Y-%m-%d") < datetime.strptime(
        ...
        for purchase in self.site_purchase_item_line_ids:
            value_list.append((0, 0, {'product_id': purchase.item_id.id,
                                      ...
                                      'state': 'confirmed',
                                      'price_unit': 1,
                                      'date_planned': self.max_expected_date,
                                      }))
    ...
    def view_invoices(self):
        ...
        print
        "wwwwwwwwwwwwwwwwwwwwwwwwwwwwww", order.order_id.invoice_ids.ids
        invoice_list = []
        # for pick in  picking:
    ...
    def create(self, vals):
        ...
        if result.name == False:
            ...
            if result.general_purchase == True:
                result.name = str('GPR-') + str(result.site.loc_barcode) + "/" + str(self.env['ir.sequence'].next_by_code('site.purchase')) or '/'
            if result.vehicle_purchase == True:
                result.name = str('VPR-') + str(result.site.loc_barcode) + "/" + self.env['ir.sequence'].next_by_code(
                    'site.purchase') or '/'
            if result.telecom_purchase == True:
                result.name = str('TPR-') + str(result.site.loc_barcode) + "/" + self.env['ir.sequence'].next_by_code(
                    'site.purchase') or '/'
            if result.interlocks_purchase == True:
                result.name = str('IPR-') + str(result.site.loc_barcode) + "/" + self.env['ir.sequence'].next_by_code(
                    'site.purchase') or '/'
            if result.site_request == True:
                result.name = str('SR-') + str(result.site.loc_barcode) + "/" + str(self.env['ir.sequence'].next_by_code('site.purchase')) or '/'
...
#                     'site.purchase') or '/'
#             if result.general_purchase == True:
#                 result.name = str('GPR-') + str(result.site.loc_barcode) + str("/") + str(self.env['ir.sequence'].next_by_code('site.purchase')) or '/'
#             if result.vehicle_purchase == True:
#                 result.name = str('VPR-') + str(result.site.loc_barcode) + "/" + self.env['ir.sequence'].next_by_code(
#                     'site.purchase') or '/'
#             if result.telecom_purchase == True:
...
#                     'site.purchase') or '/'
#             if result.site_request == True:
#                 result.name = str('SR-') + str(result.site.loc_barcode) + "/" + str(self.env['ir.sequence'].next_by_code(
#                     'site.purchase') )or '/'
#
#         if len(result.site_purchase_item_line_ids) == 0:
#             raise except_orm(_('Warning'), _('Please Add any Item'))
